        ...
        def pull_mdm_id_from_global_registry # rubocop:disable Metrics/AbcSize
          unless global_registry.id_value?
            ...
            raise GlobalRegistry::Bindings::RecordMissingGlobalRegistryId,
                  "#{model.class.name}(#{model.id}) has no #{global_registry.id_column}; will retry"
          end
          ...
          unless mdm_entity_id
            raise GlobalRegistry::Bindings::EntityMissingMdmId,
                  "GR entity #{global_registry.id_value} for #{model.class.name}(#{model.id}) has no mdm id; will retry"
          end
          # rubocop:disable Rails/SkipsModelValidations
          model.update_column(global_registry.mdm_id_column, mdm_entity_id)
        end
